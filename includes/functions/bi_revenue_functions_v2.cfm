...
<cffunction name="getProjectinatorData" output="true">
	...
	<cfquery datasource="#application.datasource2#" name="DataSelect">
		...
		<cfif application.budget_year eq "YR1">
			...
			h.FEECODE, h.FEEDESCR, h.note, h.SELGROUP, h.TRIM(ACAD_CAREER) as ACAD_CAREER, h.RES, h.ACCOUNT, h.ACCOUNT_NM, 
...
<!---<cffunction name="getAllCampus_EstRev">
	<cfquery datasource="#application.datasource2#" name="EstRevSelect">
		...
		SUM(MACHHRS_YR1) MACHHRS_YR1, SUM(t.PROJHRS_YR1) PROJHRS_YR1, 
		SUM(MACHHRS_YR2) MACHHRS_YR2, SUM(t.PROJHRS_YR2) PROJHRS_YR2, 
		...
		FROM
		  (SELECT htp.CHART, htp.HOURS, 
		     htp.MACHHRS_YR1, htp.MACHHRS_YR2, 
		     htp.PROJHRS_YR1, htp.PROJHRS_YR2, 
		     htp.ADJ_ESCL_RATE_YR1 * htp.MACHHRS_YR1 UBO_ESTREV_YR1, 
		     htp.ADJ_ESCL_RATE_YR2 * htp.MACHHRS_YR2 UBO_ESTREV_YR2, 
		     htp.ADJ_ESCL_RATE_YR1 * htp.PROJHRS_YR1 ESTREV_YR1, 
		     htp.ADJ_ESCL_RATE_YR2 * htp.PROJHRS_YR2 ESTREV_YR2
		   FROM #application.hours_to_project# htp
	...
	<cfquery dbtype="query" name="EstRevSelect_sorted">
		SELECT CHART, ACTUAL, 
		  MACHHRS_YR1, MACHHRS_YR2, 
		  PROJHRS_YR1, PROJHRS_YR2, 
...
<!---<cffunction name="getAllCampus_EstRev_byRC">
	<cfquery datasource="#application.datasource2#" name="EstRevSelect">
		...
		SUM(MACHHRS_YR1) MACHHRS_YR1, 
		SUM(MACHHRS_YR2) MACHHRS_YR2, 
		SUM(t.PROJHRS_YR1) PROJHRS_YR1, 
		SUM(t.PROJHRS_YR2) PROJHRS_YR2, 
	...
	<cfquery dbtype="query" name="EstRevSelect_sorted">
		SELECT CHART, RC_CD, RC_NM, ACTUAL, MACHHRS_YR1, MACHHRS_YR2, 
		  PROJHRS_YR1, PROJHRS_YR2, UBO_ESTREV_YR1, UBO_ESTREV_YR2, ESTREV_YR1, ESTREV_YR2
		FROM EstRevSelect
...
<cffunction name="getB325_V1_Campus_data2" output="true">
...
SELECT h.BIENNIUM as "Biennium", h.INST as "Bus Unit", h.CHART as "KFS Chart", 
	h.rptg_chart as "Rptg Chart",
	h.B2_RC as "RC Code", rcb2.RC_NM as "RC Name", h.ACCOUNT as "Account Number", h.ACCOUNT_NM as "Account Name",
	h.RCB_EXCL as "RCB Excl", h.SCHOOL  as "School", h.SELGROUP as "Tuition Group", 
	h.SELGRP_DESC as "Tuition Group Descr",
 	h.tg_excl as "TG Excl",
	h.FEECODE as "Fee Code", h.FEEDESCR as "Fee Code Descr", h.B2_FEE_DISTANCE as "Distance", 
	h.LEVEL_CD as "Career Categ", h.SLEVEL as "Level", h.SESN as "Term Code", 
	h.acad_term as "Acad Term",  
	h.b2_trm_long_desc as "Term Long Descr", h.b2_trm_shrt_desc as "Term Shrt Descr",  h.res as "Projector Residency",
	h.res_grp as "Resdcy Desc", h.b2_objcd as "Object Code", h.b2_FIN_OBJ_CD_SHRT_NM as "Object Code Shrt Name",
	h.pf_code as "ProForma Cd",
	h.pf_name as "ProForma Nm",
	h.uirr_rpt_cat as "UIRR Rpt Categ",
	h.b1_projhrs_yr2 as "Camp Proj Hrs_Yr2 Orig", h.b1_adj_escl_rate_yr2 as "Escltd Eff Rt_Yr2 Orig", 
	CASE
	  WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$')
		OR 
	...
	h.B2_HEADCOUNT as "Heads Yr2", h.B2_HOURS as "Act CH Yr2", h.b2_machhrs_yr2 as "UBO Proj Hrs_Yr2 Revsd",
	h.b2_projhrs_yr2 as "Camp Proj Hrs_Yr2 Revsd", h.b2_adj_rate as "Const Eff Rt Yr2 Revsd", 
	h.b2_adj_escl_rate_yr2 as "Escltd Eff Rt_Yr2 Revsd",  
	CASE 
	  WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$')
  		OR h.FEECODE NOT IN ('BANR$', 'BANN$', 'OVSTN$', 'OVSTR$', 'OVST', 'ACPR$', 'ACPNR$', 'G901N$', 'G901R$', 'LWARN$', 'LWARR$', 'MUSX', 'OTHER')) 
	    THEN ROUND(h.b2_adj_escl_rate_yr2 * h.b2_machhrs_yr2, 2) 
	ELSE 0 
	...
	CASE
	  WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$')
		OR 
	h.FEECODE NOT IN ('BANR$', 'BANN$', 'OVSTN$', 'OVSTR$', 'OVST', 'ACPR$', 'ACPNR$', 'G901N$', 'G901R$', 'LWARN$', 'LWARR$', 'MUSX', 'OTHER')) 
	  THEN ROUND(h.b2_adj_rate * h.b2_projhrs_yr2, 2) 
	ELSE 0 
	...
	CASE
	  WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$')
		OR 
	  h.FEECODE NOT IN ('BANR$', 'BANN$', 'OVSTN$', 'OVSTR$', 'OVST', 'ACPR$', 'ACPNR$', 'G901N$', 'G901R$', 'LWARN$', 'LWARR$', 'MUSX', 'OTHER')) 
	  THEN ROUND(h.b2_adj_escl_rate_yr2 * h.b2_projhrs_yr2, 2)
	ELSE 0 
	...
	CASE
	  WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$')
		OR 
		h.FEECODE NOT IN ('BANR$', 'BANN$', 'OVSTN$', 'OVSTR$', 'OVST', 'ACPR$', 'ACPNR$', 'G901N$', 'G901R$', 'LWARN$', 'LWARR$', 'MUSX', 'OTHER')) 
	  THEN ROUND( (h.b2_adj_escl_rate_yr2 * h.b2_projhrs_yr2) - h.b1_projhrs_yr2, 2)
	ELSE 0 
	END as "YR2_REV_CHG", 
	h.B2_RC as "RC Code2", h.RC_NM as "RC Name2", h.ACCOUNT_NM as "Account Name2"
FROM ch_user.htp h 
	  <cfif university_user eq 'N'>
		WHERE SUBSTR(H.INST,3,2) = <cfqueryparam cfsqltype="cf_sql_char" value="#campus#">
		  <cfif RC_only eq true>
		  	AND H.b2_RC = <cfqueryparam cfsqltype="cf_sql_char" value="#fee_RC#">
		  </cfif>
	  </cfif>
	</cfquery>
...
<cffunction name="getB325_V1_Campus_data" output="true">
	...
	<cfquery datasource="#application.datasource2#" name="B325_V1_Campus_data">
SELECT
h.BIENNIUM as "Biennium",
h.INST as "Bus Unit", --IUxxA excl. CO/FT
h.CHART as "KFS Chart", --equiv to COA code but incl FT (IU Ft. Wayne) & CO (IUPU Columbus)
    CAST(CASE
...
rcb2.RCB_EXCL as "RCB Excl",  -- Select value of 'Y' to capture Online Class Connect account hours
rcb2.SCHOOL  as "School",  
h.SELGROUP as "Tuition Group",
sg.DESCR as "Tuition Group Descr",
CAST(CASE WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$'))
  THEN 'N' 
  WHEN h.FEECODE IN ('BANR$', 'BANN$', 'OVSTN$', 'OVSTR$', 'OVST', 'ACPR$', 'ACPNR$', 'G901N$', 'G901R$', 'LWARN$', 'LWARR$', 'MUSX', 'OTHER', 'RHSR$') 
  -- no longer see FEEXMP
  THEN 'Y' ELSE 'N'
  END as character varying(1)) as "TG Excl",
h.FEECODE as "Fee Code",
h.FEEDESCR as "Fee Code Descr",
h.B2_FEE_DISTANCE as "Distance", -- go to AllFee db for this info, not htp 

ac.LEVEL_CD as "Career Categ",  -- collapse 7 academic careers to 3 (UGRD/GRAD/PROF)
ac.SLEVEL as "Level",  -- Undergraduate/Graduate
h.SESN as "Term Code",  -- 2 or 3-char term acronym (FL/SP/SS1/SS2/WN)
DECODE (h.SESN, 'FL', 'Fall',
              ...
              'WN', 'Winter',
              '') as "Acad Term",  
t2.Expanded_Descr as "Term Long Descr",  
t2.DESCRSHORT as "Term Shrt Descr",  h.res as "Projector Residency",
r.res_grp as "Resdcy Desc", 
h.b2_objcd as "Object Code",
ob2.FIN_OBJ_CD_SHRT_NM as "Object Code Shrt Name",
CAST(CASE WHEN h.b2_objcd IN ('0701','0801','0901','1001') THEN '09de'
WHEN h.b2_objcd IN ('0808','0908','0802','0803','0902','0903') THEN '10ms'
ELSE pf2.PROFORMA_CD 
END as character varying(4)) as "ProForma Cd",
CASE WHEN h.b2_objcd IN ('0701','0801','0901','1001') THEN 'Undifferentiated Dist Ed'
WHEN h.b2_objcd IN ('0808','0908','0802','0803','0902','0903') THEN 'Campus Mkt Share'
...
END as "ProForma Nm",
  ...
  THEN 'OVST'
  WHEN h.FEECODE IN ('OCCH$', 'OCCE$') -- not OCCI$
  THEN 'OCC'
  WHEN h.FEECODE IN ('BANR$', 'BANN$', 'OVST', 'OTHER', 'MUSX', 'OCCI$')  --modify logic 8/28/19 to also excl. OCCI$ fee code, hrs belong to COI
  THEN 'EXCL'
  ELSE 'REG'
  END as "UIRR Rpt Categ",
h.b1_projhrs_yr2 as "Camp Proj Hrs_Yr2 Orig",  -- FYxx Campus Projected Hours (hi yr orig)
h.b1_adj_escl_rate_yr2 as "Escltd Eff Rt_Yr2 Orig",  -- all populated @ 4/15/19, will be FY21 escalated rates at Recommended to BOT pct incrses (FY21)
CASE
WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$')
OR 
h.FEECODE NOT IN ('BANR$', 'BANN$', 'OVSTN$', 'OVSTR$', 'OVST', 'ACPR$', 'ACPNR$', 'G901N$', 'G901R$', 'LWARN$', 'LWARR$', 'MUSX', 'OTHER')) 
...
ELSE 0 
END as "Tuit Rev Yr2 Orig", 
h.B2_HEADCOUNT as "Heads Yr2",
h.B2_HOURS as "Act CH Yr2",  -- FY20 Actual as of Census
h.b2_machhrs_yr2 as "UBO Proj Hrs_Yr2 Revsd",  -- FYxx Enrollment Study Hours (hi yr rev)
h.b2_projhrs_yr2 as "Camp Proj Hrs_Yr2 Revsd",  -- FYxx Campus Projected Hours (hi yr rev)
h.b2_adj_rate as "Const Eff Rt Yr2 Revsd",  -- FY20 Effective Rate (Vc) using FY20 fcp data??
h.b2_adj_escl_rate_yr2 as "Escltd Eff Rt_Yr2 Revsd",  
CASE 
  WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$')
  OR h.FEECODE NOT IN ('BANR$', 'BANN$', 'OVSTN$', 'OVSTR$', 'OVST', 'ACPR$', 'ACPNR$', 'G901N$', 'G901R$', 'LWARN$', 'LWARR$', 'MUSX', 'OTHER')) 
...
THEN ROUND(h.b2_adj_rate * h.b2_projhrs_yr2, 2) 
ELSE 0 
END as "Const Tuit Rev Yr2 Revised",  -- campus proj'd rev FY21 hrs @ FY20 const eff rates
CASE
WHEN (h.CHART IN ('FT', 'IN') AND h.FEECODE IN ('BANR$', 'BANN$')
OR 
h.FEECODE NOT IN ('BANR$', 'BANN$', 'OVSTN$', 'OVSTR$', 'OVST', 'ACPR$', 'ACPNR$', 'G901N$', 'G901R$', 'LWARN$', 'LWARR$', 'MUSX', 'OTHER')) 
...
FROM ch_user.htp h 
    LEFT OUTER JOIN BUDU001.BCN_OBJ_CD_ROLLUP_GV@dss_link pf
          ON pf.FIN_OBJECT_CD = h.b1_objcd
    LEFT OUTER JOIN BUDU001.BCN_OBJ_CD_ROLLUP_GV@dss_link pf2
          ON pf2.FIN_OBJECT_CD = h.b2_objcd
    LEFT OUTER JOIN BUDU001.CHP_RCB_MATRIX_T@dss_link rcb   --get RC name, school nm, RCB_EXCL flag to capture OCC hrs
		  ON rcb.CHART = h.CHART AND rcb.RC_CD = h.b1_rc AND rcb.ACCOUNT = h.ACCOUNT
        LEFT OUTER JOIN BUDU001.CHP_RCB_MATRIX_T@dss_link rcb2   --get RC name, school nm, RCB_EXCL flag to capture OCC hrs
		  ON rcb2.CHART = h.CHART AND rcb2.RC_CD = h.b2_rc AND rcb2.ACCOUNT = h.ACCOUNT
    LEFT OUTER JOIN BUDU001.CHP_OBJ_MTRX_T@dss_link ob   --get obj cd shrt & long nm's, 2 sets obj cds
		  ON ob.FIN_OBJECT_CD = h.b1_objcd
    LEFT OUTER JOIN BUDU001.CHP_OBJ_MTRX_T@dss_link ob2   --get obj cd shrt & long nm's, 2 sets obj cds
		  ON ob2.FIN_OBJECT_CD = h.b2_objcd
    LEFT OUTER JOIN BUDU001.CHP_ACAD_CAREER_T@dss_link ac   --get career categ fr 7 acad careers & level (Undergraduate/Graduate)
		  ON ac.ACADEMIC_CAREER = h.ACAD_CAREER
    LEFT OUTER JOIN BUDU001.CHP_SEL_GRP_T@dss_link sg   --get TG descr & SELGRP_EXCL flag to capture dual credit hrs
		  ON sg.BUSINESS_UNIT = h.INST AND sg.SEL_GROUP = h.SELGROUP
    LEFT OUTER JOIN BUDU001.CHP_TERM_T@dss_link t   --get term long & shrt names
		  ON t.SESN = h.SESN AND t.STRM = h.b1_term
        LEFT OUTER JOIN BUDU001.CHP_TERM_T@dss_link  t2   --get term long & shrt names
		  ON t2.SESN = h.SESN AND t2.STRM = h.b2_term
    LEFT OUTER JOIN BUDU001.CHP_RESIDENCY_T@dss_link r   --get residency descrip's (Resident/Nonresident), are mult bx_fee_residency fields
		  ON r.res = h.res
	  <cfif university_user eq 'N'>
		WHERE SUBSTR(H.INST,3,2) = <cfqueryparam cfsqltype="cf_sql_char" value="#campus#">
		  <cfif RC_only eq true>
...
<cffunction name="getUIRRdata">
	...
	<cfquery name="e0175data" datasource="#application.datasource#">
		SELECT * FROM  ch_user.getE0175data_fn(<cfqueryparam cfsqltype="cf_sql_varchar" value="#givenInst#">)
	</cfquery>
	<cfreturn e0175data />
</cffunction>
